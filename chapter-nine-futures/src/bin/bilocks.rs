...
fn main() {
    ...
    println!("Lock should be ready for writer: {}", writer.lock.poll_lock(&mut cx).is_ready());
    println!("Lock should be ready for reader: {}", reader.lock.poll_lock(&mut cx).is_ready());
    ...
    println!("Lock should now be pending for reader: {}", reader.lock.poll_lock(&mut cx).is_pending());
    *writer_lock = 123;

    let mut lock = reader.lock.lock();
    match lock.poll(&mut cx).unwrap() {
        Async::Ready(_) => panic!("The lock should not be lockable since writer has already locked it!"),
    ...
    println!("After reuniting our locks, the final value is still: {}", reunited_value);
}
